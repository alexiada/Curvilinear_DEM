...
Version v16 radius-wall: v15 plus finite-radius wall overlap.
...
@njit(cache=False, fastmath=True)
def write_lab_frame_output(x_save, v_save, x, v, xA, omega, t):
    ...
    for i in range(x.shape[0]):
        ...
        v_save[i, 0] = v[i, 0] + shift_v0
        v_save[i, 1] = v[i, 1] + shift_v1
...
def simulate_curvilinear_particles_jit(
    ...
    write_lab_frame_output(x_out[save_id], v_out[save_id], x, v, xA, omega, f32(0.0))
    save_id += 1
    ...
    for n in range(num_steps):
        ...
        if (n + 1) % save_every == 0:
            ...
            write_lab_frame_output(x_out[save_id], v_out[save_id], x, v, xA, omega, ts)
            save_id += 1
...
def curvilinear_to_physical_particles(
    q, p, a, b, c, d, xA=(0.0, 0.0), omega=0.0, t=0.0, min_r=1.0e-6
):
    q = as_f32(q)
    p = as_f32(p)
    coef_a = as_f32(a)
    ...
    write_lab_frame_output(x_abs, v_abs, x_rel, v_rel, xA, f32(omega), f32(t))
