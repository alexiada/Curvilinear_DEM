"""
Pre- and post-processing helpers for the Fourier and hybrid rough-wall simulations.

Version v20: trims threshold curves at corners and draws them dotted in videos.
...
def _hybrid_offset_u_interval(hs, edge_id, distance, TOP, BOTTOM, HEIGHT):
    """
    Valid local-u interval for a straight-wall offset branch.

    The parallel offset of each side must be shortened near convex corners.
    Otherwise neighbouring side offsets overlap instead of meeting cleanly.
    """
    prev_edge = (edge_id - 1) % 4
    next_edge = (edge_id + 1) % 4

    _, _, tx_prev, ty_prev, _, _, _ = hs._edge_data(prev_edge, TOP, BOTTOM, HEIGHT)
    _, _, tx, ty, _, _, length = hs._edge_data(edge_id, TOP, BOTTOM, HEIGHT)
    _, _, tx_next, ty_next, _, _, _ = hs._edge_data(next_edge, TOP, BOTTOM, HEIGHT)

    c0 = np.clip((-tx_prev) * tx + (-ty_prev) * ty, -1.0, 1.0)
    c1 = np.clip((-tx) * tx_next + (-ty) * ty_next, -1.0, 1.0)

    a0 = np.arccos(c0)
    a1 = np.arccos(c1)

    m0 = distance / np.tan(0.5 * a0)
    m1 = distance / np.tan(0.5 * a1)

    if m0 + m1 >= length:
        return 0.5, 0.5

    return m0 / length, (length - m1) / length
...
def animate_physical_simulation(
    ...
    trace_label = "particle 0" if show_trace0 else "_nolegend_"
    trace0, = ax.plot([], [], linewidth=1.0, label=trace_label)
...
def animate_rough_physical_simulation(
    t,
    x,
    x_table,
    y_table,
    ...
    switch_radius=None,
    enter_factor=2.0,
    separation_samples=160,
    show_trace0=True,
    show_legend=True,
    title="rough-wall physical simulation",
):
    """
    Animate particles in laboratory physical space using a sampled rough wall.

    This is the rough-wall analogue of animate_physical_simulation. The wall is
    passed directly as x_table, y_table, so no Fourier coefficients are needed.

    If TOP, BOTTOM, HEIGHT, and switch_radius are supplied, a red curve marks
    the Cartesian/curvilinear switching threshold used by the hybrid solver:
    straight-wall distance = enter_factor * switch_radius.
    """
    xA = np.asarray(xA, dtype=float)
    x_table = np.asarray(x_table, dtype=float)
    y_table = np.asarray(y_table, dtype=float)
    boundary0 = np.column_stack((x_table, y_table))
    frame_ids = _frame_ids(t, stride)
    ...
    can_draw_threshold = (
        ...
        and switch_radius is not None
    )
    threshold0 = []
    if can_draw_threshold:
        import hybrid_solver as hs

        TOP = float(TOP)
        BOTTOM = float(BOTTOM)
        HEIGHT = float(HEIGHT)
        switch_distance = float(enter_factor) * float(switch_radius)
        ...
        for edge_id in range(4):
            ...
            local = []
            u0, u1 = _hybrid_offset_u_interval(
                hs, edge_id, switch_distance, TOP, BOTTOM, HEIGHT
            )
            for u in np.linspace(u0, u1, int(separation_samples)):
                sigma = u * length
                local.append((
                    vx + sigma * tx + switch_distance * nx,
                    vy + sigma * ty + switch_distance * ny,
                ))
            threshold0.append(np.asarray(local, dtype=float))

    all_boundary = []
    for ti in t[frame_ids]:
        all_boundary.append(boundary0 + xA * np.sin(omega * ti))
        for curve in threshold0:
            all_boundary.append(curve + xA * np.sin(omega * ti))
    all_boundary = np.vstack(all_boundary)
    ...
    boundary_line, = ax.plot([], [], linewidth=1.0, color="black", label="boundary")
    threshold_lines = []
    for k in range(len(threshold0)):
        label = "mode threshold" if k == 0 else "_nolegend_"
        line, = ax.plot(
            [],
            [],
            linewidth=1.2,
    ...
    trace_label = "particle 0" if show_trace0 else "_nolegend_"
    trace0, = ax.plot([], [], linewidth=1.0, label=trace_label)
    time_text = ax.text(0.98, 0.95, "", transform=ax.transAxes, ha="right")
    if show_legend:
    ...
    def init():
        ...
        trace0.set_data([], [])
        time_text.set_text("")
        return (boundary_line, scat, trace0, time_text, *threshold_lines)
    ...
    def update(frame):
        ...
        for line, curve in zip(threshold_lines, threshold0):
            curve_t = curve + shift
            line.set_data(curve_t[:, 0], curve_t[:, 1])
        scat.set_offsets(x[i])
...
def animate_hybrid_coordinate_simulation(
    t,
    q,
    mode,
    TOP=None,
    ...
    show_trace0=False,
    show_legend=True,
    title="hybrid coordinate simulation",
):
    """
    Animate hybrid solver coordinates.

    For curvilinear particles, q stores (edge_id, u, v). For Cartesian
    particles, q stores (-1, x, y). If the rough-wall geometry parameters are
    supplied, Cartesian particles are projected to the nearest rough wall patch
    for display, so every particle remains visible in wall-patch coordinates.

    If switch_radius is supplied, a red curve marks the Cartesian/curvilinear
    display threshold corresponding to straight-wall distance
    enter_factor * switch_radius. Points closer to the wall are below the curve.
    """
    frame_ids = _frame_ids(t, stride)

    can_project_cartesian = (
        TOP is not None
        and BOTTOM is not None
        and HEIGHT is not None
    )

    if can_project_cartesian:
        import hybrid_solver as hs
    ...
    if can_project_cartesian and switch_radius is not None:
        switch_distance = float(enter_factor) * float(switch_radius)
        for edge_id in range(4):
            xs = []
            ys = []
            ...
            for u_straight in np.linspace(u0, u1, int(separation_samples)):
                ...
                ys.append(v_sep)
            separation_curves.append((np.asarray(xs), np.asarray(ys)))
            if len(ys) > 0:
                max_v = max(max_v, float(np.nanmax(ys)) * 1.05)

    for i in frame_ids:
        pts = np.full((q.shape[1], 2), np.nan, dtype=float)
        ...
        for j in range(q.shape[1]):
            if mode[i, j] == 1:
                ...
                pts[j, 0] = edge_id + u
                pts[j, 1] = v
            elif can_project_cartesian:
                xj = q[i, j, 1]
                yj = q[i, j, 2]
                edge_id, _, _ = hs.straight_trapezium_nearest_edge(
                    xj, yj, TOP, BOTTOM, HEIGHT
                )
        ...
        if np.any(np.isfinite(pts[:, 1])):
            max_v = max(max_v, float(np.nanmax(pts[:, 1])) * 1.05)
        q_display_frames.append(pts)

    fig, ax = plt.subplots(figsize=(7, 4))
    ax.set_xlim(-0.15, 4.15)
    ax.set_ylim(-0.05, max_v)
    ax.set_xlabel("wall patch")
    ax.set_ylabel("v")
    ax.set_title(title)
    ax.grid(True)

    for edge_id in range(5):
        ax.axvline(edge_id, linewidth=0.6, color="0.75")
    ax.axhline(0.0, linewidth=0.8, linestyle="--", color="black", label="rough wall")
    separation_lines = []
    for k, (xs, ys) in enumerate(separation_curves):
        label = "mode threshold" if k == 0 else "_nolegend_"
        line, = ax.plot(
            xs,
            ys,
    ...
    def init():
        ...
        trace0.set_data([], [])
        time_text.set_text("")
        return (scat, trace0, time_text, *separation_lines)

    def update(frame):
        i = frame_ids[frame]
        pts = q_display_frames[frame]
        scat.set_offsets(pts[np.isfinite(pts[:, 0])])

        if show_trace0:
            tx = []
            ty = []
            for pts_trace in q_display_frames[:frame + 1]:
                if np.isfinite(pts_trace[0, 0]):
                    tx.append(pts_trace[0, 0])
                    ty.append(pts_trace[0, 1])
            trace0.set_data(tx, ty)
        else:
            trace0.set_data([], [])
...
def animate_curvilinear_simulation(
    ...
    wall_line, = ax.plot([], [], linewidth=1.0, label="moving wall")
    scat = ax.scatter([], [], s=s, label="particles")
...
def to_jshtml(animation, embed_limit_mb=50, fps=None, embed_frames=True):
    """
    Convert a Matplotlib animation to notebook HTML.

    Matplotlib's default embedded-animation limit is 20 MB, which is often too
    small for particle simulations. embed_limit_mb raises that limit locally for
    this conversion.
    """
    if HTML is None:
        return animation

    old_limit = plt.rcParams.get("animation.embed_limit", None)
    plt.rcParams["animation.embed_limit"] = float(embed_limit_mb)
    try:
        return HTML(animation.to_jshtml(fps=fps, embed_frames=embed_frames))
    finally:
